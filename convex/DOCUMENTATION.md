...
- [`http.ts`](http.ts) - HTTP routes mounted by `httpRouter()`: `GET /karen/health`, `POST /karen/ingest` (shared-secret OR device-token bearer), `POST /karen/auth/device/start`, `POST /karen/auth/device/poll`, `POST /karen/admin/cleanup`, `POST /karen/admin/moderate-post`, `POST /karen/admin/moderate-user`, `POST /karen/admin/reset-user`, `POST /karen/admin/org-settings`, `POST /karen/admin/wipe-placeholders`, `POST /karen/voice/synthesize`. All non-health, non-device-flow, non-voice routes go through `authorizeIngest` and call into the matching `internal.karen.*BySecret` mutation; device flow uses per-IP rate limits via `internal.karen.checkIpRateLimit`. The voice endpoint requires an allowed browser origin, can require `KAREN_VOICE_SYNTH_SECRET`, rate limits by trusted IP bucket, and only allows configured ElevenLabs voice IDs.
...
| POST | `/karen/voice/synthesize` | allowed `Origin` plus optional `x-karen-voice-secret` | Browser TTS proxy for short Karen voice clips. Requires `ELEVENLABS_API_KEY`, `KAREN_VOICE_ALLOWED_ORIGINS`, and optionally `KAREN_VOICE_SYNTH_SECRET`. |
...
- **Authed ingest identity wins.** When `/karen/ingest` authenticates with a device token, `ingestEvent` derives Clerk user/org identity from the verified user row and ignores conflicting identity fields in the request body.
- **Voice HTTP is origin-bound.** `/karen/voice/synthesize` must not return wildcard CORS. Configure explicit origins and an allowed voice-id list before enabling browser TTS.
