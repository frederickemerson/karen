...
- [`PromptCourtPanel.tsx`](PromptCourtPanel.tsx) - embedded PromptCourt surface rendered inside the OpenChamber main app when `activeMainTab === 'promptcourt'`. Lighter cousin of `PromptCourtPage`: stats summary, live run feed (`/api/promptcourt/runs/events`), recent verdicts, public feed (via `BadPromptGraveyard`), and the diff-review/quiz handoff. Hosts `DiffReviewPanel` when an active GUI run reaches `quiz_required`, then mounts `KarenQuizGameModal` on confirm. Uses inherited dark-chrome tokens (`bg-card`, `border-border`, `var(--status-*)`); the standalone `/karen` route at `PromptCourtPage` keeps the cream-on-light scoreboard shape for shareable links.
- [`DiffReviewPanel.tsx`](DiffReviewPanel.tsx) - "Don't know what you changed?" diff-review screen. Renders a per-file list with `+N/-M` counts and a fixed-width hunk pane, and exposes a "Take the read check" CTA that hands the run off to `KarenQuizGameModal`. Parses unified diff blobs client-side because the run payload already ships the raw diff.
- [`KarenQuizGameModal.tsx`](KarenQuizGameModal.tsx) - full-screen Kahoot-style quiz overlay shown when a GUI run reaches `quiz_required`. Reads the diff and questions off the run, plays a Kahoot-inspired music loop, speaks each question through the Karen ElevenLabs TTS proxy (with browser TTS fallback), and walks the user through `intro` → `question` → `reset`/`wrong`/`passed` stages. On a wrong answer it renders the full-screen red `GitResetCard` ("GIT RESET --HARD / Sandbox deleted. Real repo stays clean.") for ~3.5s before auto-dismissing; the detail explainer (`wrong` stage) is reachable from the card. Submits answers, completion, and abandonment via `submitGuiAnswer`, `completeGuiQuiz`, and `abandonGuiQuiz` from `../../../lib/promptcourt.ts`.
