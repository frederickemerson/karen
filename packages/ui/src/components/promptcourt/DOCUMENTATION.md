...
- [`PromptCourtPage.tsx`](PromptCourtPage.tsx) - the live profile page. Composes `LaunchControls`, `LiveRunStream`, `RecentSessions`, `ProfilePanel`, `BadPromptGraveyard`, `ProofProfileCard`, and the auto-mounting `KarenQuizGameModal`. Subscribes to `/api/promptcourt/runs/events` SSE for the global live run stream and, when launching a guarded run from the browser, to `/api/promptcourt/gui-runs/:runId/events` for that run's lifecycle. Auto-opens `KarenQuizGameModal` once per run when status hits `quiz_required`, including direct links such as `/karen?run=<id>`. Reads PromptCourt overview/profile data from Convex when configured and falls back to local data when not. Kept as the fallback `/karen` and `/promptcourt` standalone route.
- [`PromptCourtPanel.tsx`](PromptCourtPanel.tsx) - inline PromptCourt experience rendered inside the OpenChamber main app shell when the `promptcourt` main tab is active. Header carries the Karen logo, discipline score pill, current streak, and `KarenAuthBar`. Streams the global live runs feed via `/api/promptcourt/runs/events` SSE, shows recent verdicts off the Convex/local profile, and lists the public bad-prompt feed through `BadPromptGraveyard`. When the active guarded run (hydrated from a `?run=<id>` URL param plus per-run SSE) reaches `quiz_required`, the panel swaps the main area to `DiffReviewPanel`; clicking "Take the read check →" flips local state to render `KarenQuizGameModal`. Matches vision frame_007.
- [`DiffReviewPanel.tsx`](DiffReviewPanel.tsx) - "Don't know what you changed?" diff-review surface used inside `PromptCourtPanel` before the quiz gate. Parses the run's unified diff into file rows with +N/-M counts, supports per-file expand/collapse with monospace diff text, surfaces a warning row ("Karen will ask you 5 questions about this diff. Wrong = git reset --hard."), and exposes the "Take the read check →" CTA via the `onStartQuiz` prop. Matches vision frame_007.
- [`KarenQuizGameModal.tsx`](KarenQuizGameModal.tsx) - full-screen Kahoot-style quiz overlay shown when a GUI run reaches `quiz_required`. Reads the diff and questions off the run, plays a Kahoot-inspired music loop, speaks each question through the Karen ElevenLabs TTS proxy (with browser TTS fallback), and walks the user through `intro` → `question` → `reset`/`wrong`/`passed` stages. On a wrong answer the modal flashes a full-screen red `GitResetCard` ("REDO UR WORK" / `GIT RESET --HARD` / "Sandbox deleted. Real repo stays clean.") for ~3.5s before transitioning into the existing review panel. Submits answers, completion, and abandonment via `submitGuiAnswer`, `completeGuiQuiz`, and `abandonGuiQuiz` from `../../../lib/promptcourt.ts`. Reset card matches vision frames 009 and 015.
...
- `PromptCourtPage` - the standalone live profile page (`/karen`, `/promptcourt`, `/u/:username`). Optional `username` prop; falls back to the Clerk identity when configured.
- `PromptCourtPanel` - inline PromptCourt surface mounted inside the OpenChamber app shell when the `promptcourt` main tab is active. No props.
- `DiffReviewPanel` - read-before-promote diff list with quiz CTA; consumed by `PromptCourtPanel` and reusable for anywhere a guarded run is in scope. Props: `{ run, onStartQuiz }`.
