...
- CLI-local modules live under [`bin/`](bin/) and [`lib/`](lib/). [`bin/karen.js`](bin/karen.js) owns terminal UX, guarded OpenCode entrypoints, worktree orchestration, and the quiz flow; [`lib/opencode-hook.js`](lib/opencode-hook.js) owns the upstream-hook vs PTY interception decision.
...
- [`lib/karen-auth.js`](lib/karen-auth.js) - local device-token auth helpers. Owns loading/saving Karen auth state and the Convex device-link flow used by the setup wizard.
- [`lib/karen-fx.js`](lib/karen-fx.js) - terminal/audio effect helpers shared by the shell and tests. Keep presentation-only side effects here when they do not need prompt/session state.
- [`lib/karen-rewrite.js`](lib/karen-rewrite.js) - OpenAI-backed prompt rewriter for BLOCKED prompts. Exports `karenRewritePrompt`, `rewriteAiAllowed`, `rewriteModel`, `gatherRepoContext`, and `formatRewriteForDisplay`. Uses the Responses API with strict JSON schema (same pattern as `quiz.js`); honors `OPENAI_API_KEY`, `KAREN_REWRITE_AI`, `KAREN_REWRITE_MODEL` (falls back to `KAREN_QUIZ_MODEL`), and `KAREN_REWRITE_TIMEOUT_MS`. Falls back to the static `evaluation.suggestedRewrite` when no key or call fails.
- [`lib/karen-timing.js`](lib/karen-timing.js) - perf instrumentation + per-session profile cache. Exports `isTimingEnabled`, `timeSync`, `timeAsync`, `getCachedProfile`, `invalidateProfileCache`, and `getProfileCacheStats`. Activated by `KAREN_VERBOSE_TIMING=1`; cache TTL via `KAREN_PROFILE_CACHE_TTL_MS` (default 5000ms). Wrappers are no-ops when timing is disabled.
- [`lib/karen-tui-guard.js`](lib/karen-tui-guard.js) - overlay UX for the `karen /tui` PTY wrapper. Exports `renderGuardBadge`, `renderBlockedSidebar`, `formatBlockedSidebar`, `createScreenTailBuffer`, `updateScreenTail`, `screenTailRaw`, `screenTailVisible`, `stripAnsi`, and `openCodePluginAvailability`. Paints a "karen guard" pill in the top-left and a PromptCourt CHECK sidebar on BLOCK; both gated by TTY plus `KAREN_TUI_BADGE` / `KAREN_TUI_SIDEBAR` envs. The screen-tail ring buffer replaces the previous string-slice approach that degraded after model-picker churn.
- [`lib/karen-voice.js`](lib/karen-voice.js) - shared Karen voice lines and ElevenLabs voice request helpers for terminal cues. Keep browser TTS UI in `packages/ui/src/components/promptcourt/`.
...
Inside the shell, slash commands include `/help`, `/setup`, `/gui`, `/tui`, `/tui-raw`, `/run`, `/providers`, `/models`, `/auth`, `/mcp`, `/agent`, `/session`, `/stats`, `/audio`, `/feed`, `/profile`, `/diff`, `/opencode ...`, and `/quit`. The full list is enumerated in `printHelp` and `printOpenCodeCommands` in [`bin/karen.js`](bin/karen.js). `/run <prompt>` uses the guarded prompt -> isolated worktree flow. Raw OpenCode passthrough commands (`/tui-raw`, `/opencode`, `/oc`) are disabled unless `KAREN_ALLOW_RAW_OPENCODE=1` is set for an explicit local escape hatch. The `/gui` command starts the OpenChamber web UI on `OPENCHAMBER_PORT` and prints the **editor** URL (`/` by default, overridable via `KAREN_GUI_PATH`); it also prints `/karen` for the PromptCourt-only scoreboard route ([`packages/ui/src/App.tsx`](../ui/src/App.tsx) maps `/karen` to `PromptCourtPage`, not `MainLayout`).
...
- **Worktree isolation.** Approved prompts execute in a temp worktree created by `createIsolatedWorktree`. Failed quizzes never touch the real repo. Tracked changes and untracked files from the real repo are mirrored into the worktree as a baseline so OpenCode sees the same state. Karen rejects symlinks that resolve outside the worktree and discards a run if OpenCode moves the isolated worktree `HEAD`.
- **Guarded OpenCode execution.** OpenCode `run` inherits Karen's git commit guard PATH wrapper and runtime hook env. The CLI still checks `HEAD` afterward because absolute `git` paths and custom tooling can bypass PATH-level wrapping.
