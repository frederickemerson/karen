...
- `packages/web/server/lib/git/`: Git module directory containing all Git-related functionality.
  ...
  - `routes.js`: Express route registration for `/api/git/*` endpoints (including `POST /api/git/commit-read-quiz`, which wraps PromptCourt [`quiz.js`](../promptcourt/quiz.js) `buildQuiz` for the Git panel commit-read modal).
