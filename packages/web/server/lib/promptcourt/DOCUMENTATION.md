...
- [`evaluator.js`](evaluator.js) - prompt scoring. Exports `evaluatePrompt(prompt)` returning `{ score, verdict, allowed, intent, promptIntent, reasons, chips, dimensions, suggestedRewrite }`, `classifyPromptIntent(prompt)` returning `'commit' | 'normal'`, plus `extractPromptText(body)` for OpenCode-shaped request bodies. A fast-path classifier short-circuits conversational greetings (`intent='conversational'`) and read-only exploration prompts (`intent='exploration'`) to `approved` without scoring; everything else falls through to the dimensional scorer where score < ~25 (or hopeless without concrete intent) produces `verdict='blocked'`. On top of the general gate, a commit-intent layer fires when the prompt is asking to commit, push, open a PR, or merge: if it omits a diff explanation, named tests, or a blast-radius claim, the verdict is forced to `blocked` and commit-specific chips are added. `reasons` (string[]) is the legacy unstructured explanation; `chips` (typed array of `{ id, label, severity, category }`) is the structured form callers should prefer.
...
- [`cloud.js`](cloud.js) - Convex sync. Exports `createPromptCourtCloudSync({ env, fetchImpl, loadEnvFiles })` returning `{ enabled, send, recordBlockedPrompt, recordApprovedPrompt, recordQuizResult, getPendingCount, flushNow }`. Reads `KAREN_CLOUD_SYNC`, `CONVEX_HTTP_ACTIONS_URL` (and Vite/site aliases), and `KAREN_CLOUD_INGEST_SECRET`. POSTs JSON to `<endpoint>/karen/ingest` with `Authorization: Bearer <secret>` and a stable `eventId` for Convex-side idempotency. Failed writes go through a bounded in-memory retry queue (cap 500, exponential backoff up to 30s, non-retryable 4xx responses are dropped with `cloud_sync_dropped` warning). Errors never throw to callers and are only logged at info level when `KAREN_CLOUD_DEBUG=1`.
...
- [`replay-video.js`](replay-video.js) - replay-tape video contract + renderers. Exports `REPLAY_VIDEO_SCHEMA_VERSION`, `REPLAY_COMPOSITION_ID`, `REMOTION_INSTALL_HINT`, `RemotionNotInstalledError`, `buildReplayStepsFromEvents`, `normalizeReplaySteps`, `buildReplayVideoContract`, `createStubReplayRenderer`, `createRemotionReplayRenderer`, `createReplayVideoRenderer`, and `renderReplayVideoExport`. The Remotion renderer is selected when `KAREN_REPLAY_RENDERER=remotion`; if the optional `remotion`/`@remotion/*` packages are not installed it throws `RemotionNotInstalledError` (HTTP 503 in `replay-video-routes.js`). Otherwise a stub JSON renderer ships a Remotion-ready manifest.
...
HTTP endpoints mounted by [`registerPromptCourtRoutes`](routes.js). Non-admin PromptCourt endpoints require the local PromptCourt session token through `Authorization: Bearer <KAREN_PROMPTCOURT_SESSION_TOKEN>` or the `karen_promptcourt_session` cookie. Karen page navigations and `GET /api/promptcourt/session` receive that cookie as `HttpOnly`, `SameSite=Lax`, and same-origin scoped so the local UI can call protected routes without exposing the token to JavaScript. When the env var is not set, the server generates a per-process token without logging the secret value. Clerk bearer tokens are accepted when `CLERK_SECRET_KEY` is configured.
...
- **Redaction runs before any cross-boundary write.** Routes call `redactPublicText` before storing posts. `storage.js` also redacts run-event labels/details so direct store callers cannot accidentally persist raw secrets.
- **Private local state.** `storage.js` writes `promptcourt.json` as `0600`, keeps its directory at `0700`, and refuses symlinked state paths. Partial-write corruption of `promptcourt.json` is not tolerated.
- **Atomic writes.** `storage.js` writes to a temp file and renames.
...
type Chip = {
  id:
    ...
    | 'vague-language'
    | 'lazy-prompt-warning';
  label: string;       // short, display-ready (e.g. "No diff explanation")
  severity: 'critical' | 'warn';
  category: 'commit' | 'general' | 'long-prompt' | 'secrets';
};
...
type Evaluation = {
  ...
  allowed: boolean;
  intent: 'conversational' | 'exploration' | null;   // fast-path intent
  promptIntent: 'commit' | 'normal';                  // commit classifier
  reasons: string[];                                  // legacy, unstructured
  chips: Chip[];                                      // structured replacement
  dimensions: Record<string, number>;
  suggestedRewrite: string;
};
```

`reasons` is preserved for backward compatibility (Karen CLI's `printVerdict`, recorded `failureReasons`, redacted cloud payloads). New UI surfaces should consume `chips`.
...
- Adding a new chip means adding the id to `CHIP_DEFS` in [`evaluator.js`](evaluator.js) (with label, severity, category), wiring it from the appropriate check, and updating the `Chip` union in this doc plus [`evaluator.test.js`](evaluator.test.js).
- Commit-specific checks live in `evaluator.js` and run only when `classifyPromptIntent(prompt) === 'commit'`. False positives (e.g. "commit it to memory", "push notifications") must be filtered by `COMMIT_NEGATIVE_PATTERNS`.
